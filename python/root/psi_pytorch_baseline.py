...
def psilong_pytorch_gpu(attention_scores: torch.Tensor, bus_memory: torch.Tensor,
    ...
    for d in range(depth):
        ...
        # Since gather, need to handle invalid
        ...
        if valid_idx.numel() > 0:
            ...
            t_weight = torch.exp(torch.tensor(-0.3 * d, dtype=torch.float32, device='cuda'))
...
def benchmark_pytorch_gpu(n: int, depth: int, num_warmup: int = 20, num_runs: int = 100) -> dict:
...
if __name__ == "__main__":
    ...
    # Benchmark
    configs = [(1024, 5), (4096, 5), (16384, 5)]
    results = {}
    for n, depth in configs:
        print(f"Benchmarking n={n}, depth={depth}")
        res = benchmark_pytorch_gpu(n, depth)
        results[(n, depth)] = res
    ...
    with open('pytorch_gpu_results.json', 'w') as f:
