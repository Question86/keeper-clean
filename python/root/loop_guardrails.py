...
try:
    ...
    AIIntegrityProtector = None

try:
    from ai_false_positive_suppressor import AIFalsePositiveSuppressor
except ImportError:
    AIFalsePositiveSuppressor = None


REF_RE = re.compile(r"\[ref:([^\]]+)\]")
TASK_ID_RE = re.compile(r"TASK_(\d{4})")
REPORT_BASENAME_RE = re.compile(r"^report_(TASK_\d{4})_L(\d+)_v(\d+)\.md$")
ARCHIV_RE = re.compile(r"^ARCHIV_(\d{4})\.md$")

GATE_AUTOSTART_STATE_PATH = Path("logs/loop_gate_autostart_state.json")
GATE_AUTOSTART_COOLDOWN_SECONDS = 120
GATE_AUTOSTART_SCRIPTS = [
    "scripts/start_autostart_services.py",
    "scripts/milestone_knowledge_monitor.py",
    "scripts/autonomous_audit_system.py",
    "scripts/generate_optimized_session_pack.py",
    "scripts/api_audit_comprehensive.py",
    "scripts/knowledge_health_monitor.py",
    "scripts/update_reminder_engine.py",
    "behavioral_telemetry_analyzer.py",
]
_CLOSE_TASK_LOCK = threading.Lock()
...
def _write_text_atomic_guardrails(path: Path, content: str) -> None:
    """Best-effort atomic file write for guardrails operations."""
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _launch_script_detached(script_path: Path, workspace_root: Path) -> None:
    creationflags = 0
...
def maybe_trigger_gate_autostart(workspace_root: Path, status: Any, loop_num: Any) -> Dict[str, Any]:
    """Best-effort script autostart when loop gate runs in ACTIVE loop.

    This is intentionally non-blocking and never changes gate verdict.
    """
    if status != "ACTIVE" or not isinstance(loop_num, int):
        return {"attempted": False, "triggered": False, "message": "Skipped (status not ACTIVE or loop invalid)"}

    state_path = workspace_root / GATE_AUTOSTART_STATE_PATH
    state = _safe_read_json(state_path)
    now = datetime.now(timezone.utc)
    now_ts = now.timestamp()

    last_loop_started = state.get("last_loop_started")
    if isinstance(last_loop_started, int) and last_loop_started == loop_num:
        return {"attempted": False, "triggered": False, "message": f"Skipped (already started in loop {loop_num})"}
    ...
    if last_attempt_ts is not None and (now_ts - last_attempt_ts) < GATE_AUTOSTART_COOLDOWN_SECONDS:
        wait_left = int(GATE_AUTOSTART_COOLDOWN_SECONDS - (now_ts - last_attempt_ts))
        return {"attempted": False, "triggered": False, "message": f"Skipped (cooldown {wait_left}s)"}

    launched: List[str] = []
    missing: List[str] = []
    failed: List[str] = []

    for rel_script in GATE_AUTOSTART_SCRIPTS:
        script_path = workspace_root / rel_script
        if not script_path.exists():
            missing.append(rel_script)
            continue
        try:
            _launch_script_detached(script_path, workspace_root)
            launched.append(rel_script)
        except Exception:
    ...
    state_update: Dict[str, Any] = {
        ...
        "last_attempt_loop": loop_num,
        "last_launched": launched,
        "last_missing": missing,
        "last_failed": failed,
    }
    if launched:
        state_update["last_loop_started"] = loop_num

    write_json(state_path, state_update)

    if not launched and (missing or failed):
        return {
            "attempted": True,
            "triggered": False,
            "message": f"No scripts launched (missing={len(missing)} failed={len(failed)})",
        }
    return {
        "attempted": True,
        "triggered": bool(launched),
        "message": f"Launched {len(launched)} script(s), missing={len(missing)}, failed={len(failed)}",
        "launched": launched,
        "missing": missing,
        "failed": failed,
    }
...
def session_pack_markdown(workspace_root: Path) -> str:
    ...
    # TASK_0245: Include bounded DB-derived startup briefing (non-blocking).
    briefing = startup_db_briefing_markdown(workspace_root, max_chars=1800)
    lines.extend(briefing.splitlines())
    lines.append("")
...
def startup_db_briefing_markdown(
    workspace_root: Path,
    max_chars: int = 1800,
    max_lessons: int = 6,
    max_reports: int = 5,
) -> str:
    """Return a compact DB-backed startup briefing for bootstrap/session context.

    This function is intentionally non-fatal: on any error it returns a short
    informational section so bootstrap/session flows never block.
    """
    lines: List[str] = []
    lines.append("## DB KNOWLEDGE BRIEFING (BOUNDED)")
    lines.append("")

    db_path = workspace_root / "keeper_knowledge.db"
    if not db_path.exists():
        lines.append("- Knowledge DB unavailable (`keeper_knowledge.db` not found).")
        lines.append("- Fallback: rely on NEU.md + recent reports.")
        return "\n".join(lines)
    ...
    try:
        ...
        lines.append("- Fallback: continue without DB briefing.")
        return "\n".join(lines)
    ...
    try:
        ...
        lesson_rows = conn.execute(
            """
            SELECT category, source_id, loop_num, lesson_text
            FROM lessons
            ORDER BY loop_num DESC, id DESC
            LIMIT ?
            """,
            (max_lessons,),
        ).fetchall()
        if lesson_rows:
            for row in lesson_rows:
                lines.append(
                    f"- L{int(row['loop_num'] or 0)} [{row['category'] or 'info'}] "
                    f"{row['source_id'] or 'unknown'}: {_clip(row['lesson_text'] or '', 120)}"
                )
        else:
            lines.append("- No lesson rows found.")
        lines.append("")

        lines.append("### Recent Reports")
        report_rows = conn.execute(
            """
            SELECT id, task_id, loop_num, goal
            FROM reports
            ORDER BY loop_num DESC, indexed_at DESC
            LIMIT ?
            """,
            (max_reports,),
        ).fetchall()
        if report_rows:
            for row in report_rows:
                lines.append(
                    f"- L{int(row['loop_num'] or 0)} {row['task_id'] or row['id']}: {_clip(row['goal'] or '', 110)}"
                )
        else:
            lines.append("- No report rows found.")
        lines.append("")

        lines.append("### Failure/Warning Patterns")
        pattern_rows = conn.execute(
            """
            SELECT category, lesson_text, loop_num
            FROM lessons
            WHERE category IN ('failure', 'warning')
            ORDER BY loop_num DESC, id DESC
            LIMIT 4
            """
        ).fetchall()
        if pattern_rows:
            for row in pattern_rows:
                lines.append(
                    f"- L{int(row['loop_num'] or 0)} [{row['category']}]: {_clip(row['lesson_text'] or '', 120)}"
                )
        else:
            lines.append("- No recent failure/warning patterns found.")
    except Exception as e:
    ...
    if len(rendered) > max_chars:
        rendered = rendered[: max_chars - 40].rstrip() + "\n- ... (briefing truncated to budget)"
    return rendered
...
def close_task(task_id: str, workspace_root: Path, summary: str = "", loop_num: int = None) -> Dict[str, Any]:
    ...
    """
    def _ensure_alt_loop_section(content: str, current_loop: int) -> Tuple[str, bool]:
        loop_section = f"## COMPLETED (LOOP {current_loop})"
        if loop_section in content:
            return content, False
        insertion = f"\n{loop_section}\n\n"
        marker = "\nEND OF DOCUMENT"
        if marker in content:
            idx = content.index(marker)
            return content[:idx].rstrip() + insertion + content[idx:], True
        return content.rstrip() + insertion + "\n", True

    def _remove_task_block_from_neu(content: str, normalized_task_id: str) -> Tuple[str, bool]:
        lines = content.splitlines()
        out: List[str] = []
        removed = False
        i = 0
        needle = f"task_{normalized_task_id}.md"
        while i < len(lines):
            line = lines[i]
            if needle in line:
                removed = True
    ...
    diagnostics: Dict[str, Any] = {
        "actions": [],
        "idempotent": False,
        "loopSectionCreated": False,
        "neuTaskRemoved": False,
        "altTaskInserted": False,
    }
    closed_at = utc_now_iso()

    with _CLOSE_TASK_LOCK:
        if loop_num is None:
            state = read_json(workspace_root / "current.json").get("STATE", {})
        ...
        reports_dir = workspace_root / "reports"
        report_candidates = sorted(reports_dir.glob(f"report_{task_id}_*.md")) if reports_dir.exists() else []
        ...
        # Snapshot current state for deterministic reconciliation and rollback.
        orig_task = read_text(task_spec_path)
        orig_alt = read_text(alt_path) if alt_path.exists() else "# Alt\n\n---\n\nEND OF DOCUMENT\n"
        orig_neu = read_text(neu_path) if neu_path.exists() else "# NEU\n\n---\n\nEND OF DOCUMENT\n"
        orig_current = read_json(current_path) if current_path.exists() else {"STATE": {}}

        in_alt = f"task_{task_id}.md" in orig_alt
        in_neu = f"task_{task_id}.md" in orig_neu
        already_completed = bool(re.search(r"STATUS:\s*COMPLETED", orig_task))
        if already_completed and in_alt and not in_neu:
            diagnostics["idempotent"] = True
            diagnostics["actions"].append("already_reconciled")
            return {
                "success": True,
                "taskId": task_id,
                "closedAt": closed_at,
                "changes": [f"{task_id} already closed and reconciled"],
                "diagnostics": diagnostics,
                "idempotent": True,
            }

        # Prepare task spec content.
        task_new = orig_task
        if "STATUS:" in task_new:
            task_new = re.sub(r"STATUS:\s*\w+", "STATUS: COMPLETED", task_new)
        else:
            task_new = re.sub(r"(MODE:\s*[^\n]+)", r"\1\nSTATUS: COMPLETED", task_new)
        if "COMPLETED: TBD" in task_new:
            task_new = task_new.replace("COMPLETED: TBD", f"COMPLETED: {closed_at}")
        elif "COMPLETED:" not in task_new:
            task_new = re.sub(r"(CREATED:\s*[\d\-T:Z]+)", f"\\1\nCOMPLETED: {closed_at}", task_new)

        # Prepare NEU removal.
        neu_new, removed_from_neu = _remove_task_block_from_neu(orig_neu, task_id)
        diagnostics["neuTaskRemoved"] = removed_from_neu

        # Prepare Alt section and insertion.
        alt_work, section_created = _ensure_alt_loop_section(orig_alt, int(loop_num))
        diagnostics["loopSectionCreated"] = section_created
        report_ref = f"reports/{report_path.name}"
        if f"task_{task_id}.md" not in alt_work:
            entry = (
                f"[ref:tasks/task_{task_id}.md|v:1|tags:completed,loop{loop_num}|src:loop{loop_num}] - {task_id}\n"
                f"  Report: [ref:{report_ref}|v:1|tags:report|src:system]\n"
                f"  Status: COMPLETED\n"
                f"  ClosedAt: {closed_at}\n"
                f"  Summary: {summary or 'Task completed.'}\n"
            )
            loop_section = f"## COMPLETED (LOOP {loop_num})"
            alt_new = alt_work.replace(loop_section, f"{loop_section}\n\n{entry}", 1)
            diagnostics["altTaskInserted"] = True
        else:
            alt_new = alt_work

        # Update current state.
        current_new = orig_current
        current_new.setdefault("STATE", {})
        current_new["STATE"]["lastTaskWorked"] = task_id
        current_new["STATE"]["lastUpdate"] = closed_at
        current_json_new = json.dumps(current_new, indent=2, ensure_ascii=False) + "\n"

        writes: List[Tuple[Path, str]] = [
            (task_spec_path, task_new),
            (alt_path, alt_new),
            (neu_path, neu_new),
            (current_path, current_json_new),
        ]

        originals: Dict[Path, str] = {
            task_spec_path: orig_task,
            alt_path: orig_alt,
            neu_path: orig_neu,
            current_path: json.dumps(orig_current, indent=2, ensure_ascii=False) + "\n",
        }
        written: List[Path] = []

        try:
            for path, content in writes:
                _write_text_atomic_guardrails(path, content)
                written.append(path)
        except Exception as e:
            for rollback_path in reversed(written):
                try:
                    _write_text_atomic_guardrails(rollback_path, originals[rollback_path])
                except Exception:
                    pass
            return {
                "success": False,
                ...
                "changes": changes,
                "diagnostics": diagnostics,
                "error": f"Atomic close-task reconciliation failed and was rolled back: {e}",
            }

        if task_new != orig_task:
            changes.append("Updated task spec STATUS to COMPLETED")
            diagnostics["actions"].append("task_status_updated")
        if diagnostics["loopSectionCreated"]:
            changes.append(f"Created missing Alt.md section for loop {loop_num}")
            diagnostics["actions"].append("alt_loop_section_created")
        if diagnostics["altTaskInserted"]:
            changes.append(f"Inserted {task_id} into Alt.md with report linkage")
            diagnostics["actions"].append("alt_task_inserted")
        if diagnostics["neuTaskRemoved"]:
            changes.append(f"Removed {task_id} from NEU.md queue")
            diagnostics["actions"].append("neu_task_removed")
        changes.append("Updated current.json lastTaskWorked")
        diagnostics["actions"].append("current_state_updated")
...
def _validate_docs_directory(workspace_root: Path, docs_dir: Path) -> Dict[str, Any]:
    ...
    for doc_path in docs_dir.iterdir():
        ...
        # AI FALSE-POSITIVE DETECTION: Check for pretense patterns in content
        try:
            from ai_integrity_protector import AIIntegrityProtector
            protector = AIIntegrityProtector(workspace_root)
            false_positive_checks = protector.detect_false_positive_patterns(content, str(doc_path.relative_to(workspace_root)))

            for check in false_positive_checks:
                if check.status == "WARN":
                    warnings.append({
                        "code": "FALSE_POSITIVE_PATTERN",
                        "message": f"Potential false-positive pretense in {doc_path.name}: {check.message}",
                        "hint": "Review content for AI pretense patterns - may indicate fabricated claims"
                    })
        except ImportError:
            # Integrity module not available, skip false-positive detection
            pass
        except Exception as e:
            warnings.append({
                "code": "FALSE_POSITIVE_CHECK_FAILED",
                "message": f"False-positive detection failed for {doc_path.name}: {e}",
                "hint": "Manual review may be required"
            })

    errors: List[Dict[str, str]] = []
    warnings: List[Dict[str, str]] = []

    # AI INTEGRITY PROTECTION: Check for bypass attempts and false-positives
    try:
        from ai_integrity_protector import AIIntegrityProtector
        protector = AIIntegrityProtector(workspace_root)

        # Check file integrity
        integrity_checks = protector.validate_file_integrity()
        for check in integrity_checks:
            if check.status == "FAIL":
                warnings.append({
                    "code": "INTEGRITY_VIOLATION",
                    "message": f"File integrity violation in {check.file_path}: {check.message}",
                    "hint": "Possible AI bypass attempt - manual verification required"
                })
            elif check.status == "WARN":
                warnings.append({
                    "code": "INTEGRITY_WARNING",
                    "message": f"Integrity warning for {check.file_path}: {check.message}",
                    "hint": "Monitor for potential integrity issues"
                })
        ...
        for check in bypass_checks:
            if check.status == "FAIL":
                warnings.append({
                    "code": "BYPASS_ATTEMPT",
                    "message": f"Bypass attempt detected: {check.message}",
                    "hint": "AI may be attempting to manipulate canonical state - investigate immediately"
                })

    except ImportError:
        warnings.append({
            "code": "INTEGRITY_MODULE_MISSING",
            "message": "AI integrity protection module not available",
            "hint": "Install ai_integrity_protector.py for enhanced security"
        })
    except Exception as e:
        warnings.append({
            "code": "INTEGRITY_CHECK_FAILED",
            "message": f"Integrity check failed: {e}",
            "hint": "Manual integrity verification may be required"
        })
...
def metadata_lint(workspace_root: Path) -> Dict[str, Any]:
    ...
    # AI Integrity Protection: Validate file integrity and check for bypass attempts
    if AIIntegrityProtector:
        try:
            protector = AIIntegrityProtector(workspace_root)
            integrity_checks = protector.validate_file_integrity()
            bypass_checks = protector.check_for_bypass_attempts()

            # Add integrity violations as warnings (non-blocking for finalization)
            for check in integrity_checks + bypass_checks:
                if check.status == "FAIL":
                    warnings.append({
                        "code": "INTEGRITY_VIOLATION",
                        "message": f"AI Integrity check failed: {check.message}",
                        "hint": "Check for unauthorized modifications or bypass attempts"
                    })
                elif check.status == "WARN":
                    warnings.append({
                        "code": "INTEGRITY_WARNING",
                        "message": f"AI Integrity warning: {check.message}",
                        "hint": "Monitor for potential integrity issues"
                    })
        except Exception as e:
            errors.append({
                "code": "INTEGRITY_CHECK_ERROR",
                "message": f"Failed to perform integrity checks: {e}",
                "hint": "Ensure ai_integrity_protector.py is available and functional"
            })

    # AI False-Positive Suppression: Validate against architectural pretense controls
    if AIFalsePositiveSuppressor:
        try:
            suppressor = AIFalsePositiveSuppressor(workspace_root)
            
            # Test the suppressor with sample content to ensure it's working
            test_result = suppressor.validate_ai_output(
                content="This is a test validation that should pass basic checks.",
                output_type="validation",
                ai_source="system_test",
                target_path="test_path",
                require_proof=False
            )
            
            if test_result.get('status') not in ['approved', 'warning']:
                warnings.append({
                    "code": "FALSE_POSITIVE_SUPPRESSOR_ERROR",
                    "message": f"AI False-Positive Suppressor validation failed: {test_result.get('status')}",
                    "hint": "Check ai_false_positive_suppressor.py implementation"
                })
            
            # Check for blocked claims
            blocked_claims = suppressor.enforcement_layer.get_blocked_claims()
            if blocked_claims:
                warnings.append({
                    "code": "BLOCKED_CLAIMS_PRESENT",
                    "message": f"Found {len(blocked_claims)} blocked claims from false-positive suppression",
                    "hint": "Review architectural enforcement log for blocked AI outputs"
                })
                
        except Exception as e:
            errors.append({
                "code": "FALSE_POSITIVE_SUPPRESSOR_ERROR",
                "message": f"AI False-Positive Suppressor failed: {e}",
                "hint": "Ensure ai_false_positive_suppressor.py is available and functional"
            })
    ...
    # Orphaned reports not referenced by Alt.md or COMPLETED_TASKS_ARCHIVE.md (incident reports excluded)
    alt_content = read_text(workspace_root / "Alt.md") if (workspace_root / "Alt.md").exists() else ""
    neu_content = read_text(workspace_root / "NEU.md") if (workspace_root / "NEU.md").exists() else ""
    archive_content = read_text(workspace_root / "docs" / "COMPLETED_TASKS_ARCHIVE.md") if (workspace_root / "docs" / "COMPLETED_TASKS_ARCHIVE.md").exists() else ""
    ...
    for p in list_report_files(workspace_root):
        ...
        if (rel not in alt_report_ref_paths) and (p.name not in alt_report_ref_names):
            warnings.append({
                "code": "ORPHAN_REPORT",
                "message": f"Report not referenced in Alt.md or COMPLETED_TASKS_ARCHIVE.md: {rel}",
                "hint": "Add the report ref under the corresponding task in Alt.md or COMPLETED_TASKS_ARCHIVE.md (or confirm it is intentional)",
            })
    ...
    for task_file in list_task_spec_files(workspace_root):
        ...
        if (rel not in referenced_task_paths) and (task_file.name not in referenced_task_names):
            warnings.append({
                "code": "ORPHAN_TASK_SPEC",
                "message": f"Task spec not referenced in NEU.md, Alt.md, or COMPLETED_TASKS_ARCHIVE.md: {rel}",
                "hint": "Add a [ref:...] entry to NEU.md (active), Alt.md (closed/blocked), COMPLETED_TASKS_ARCHIVE.md (archived), or delete if it was created by mistake",
            })
...
def generate_loop_gate(workspace_root: Path, checked_by: str = "loop_cockpit", reason: str = "automation") -> Dict[str, Any]:
    ...
    if not state_ok:
        ...
        # Non-blocking bootstrap of high-value monitoring services for active loops.
        try:
            autostart_result = maybe_trigger_gate_autostart(workspace_root, status, loop_num)
            checks.append({"id": "AUTOSTART", "ok": True, "message": autostart_result.get("message", "No message")})
        except Exception as e:
            checks.append({"id": "AUTOSTART", "ok": True, "message": f"Autostart unavailable: {e}"})
    ...
    # Bootstrap exit validation (TASK_0198: Prevent future bootstrap failures)
    # Skip if status is READY_FOR_RESET (bootstrap not yet completed)
    if state.get("status") != "READY_FOR_RESET":
        try:
            from bootstrap_exit_validator import BootstrapExitValidator
            validator = BootstrapExitValidator(workspace_root)
            bootstrap_results = validator.validate_bootstrap_exit()

            # Only block if there are critical failures (not just warnings)
            critical_failures = sum(1 for v in bootstrap_results["validations"].values()
                                   if v["status"] in ["FAIL", "ERROR"])

            if critical_failures > 0:
                blocked = True
                checks.append({"id": "BOOTSTRAP_EXIT", "ok": False,
                              "message": f"Bootstrap exit validation failed: {critical_failures} critical issue(s)"})
            else:
                checks.append({"id": "BOOTSTRAP_EXIT", "ok": True,
                              "message": "Bootstrap exit validation passed"})

        except Exception as e:
            # Non-blocking if validator fails to load/run
            checks.append({"id": "BOOTSTRAP_EXIT", "ok": True,
                          "message": f"Bootstrap exit validation unavailable: {e}"})
    else:
        checks.append({"id": "BOOTSTRAP_EXIT", "ok": True,
                      "message": "Skipped (bootstrap in progress)"})
    ...
    lines.append("---")
    lines.append("")
    lines.append("## ACTIVE LOOP REMINDERS")
    lines.append("")
    lines.append("### 📝 Littleboot Method")
    lines.append("**REQUIRED FOR FINALIZATION: Littleboot.md must exist at `d:\\Keeper-Clean-Loop1\\Littleboot.md`**")
    lines.append("- Read canonical method: [ref:docs/OPS_PROTOCOLS.md#LITTLEBOOT_METHOD|v:1|tags:protocol,littleboot|src:docs]")
    lines.append("- Purpose: Transfer current loop wisdom to next loop bootstrap")
    lines.append("- Timing: Created automatically when you call `/api/finalize-loop`")
    lines.append("- Content: Current project state, lessons learned, priority tasks")
    lines.append("- **VALIDATION**: Finalization blocked without Littleboot.md (hardcoded path check)")
    lines.append("")
    lines.append("### 🎯 Finalization Checklist")
    lines.append("- [ ] All critical tasks addressed")
    lines.append("- [ ] Lessons learned documented in reports")
    lines.append("- [ ] Next priorities clear in NEU.md")
    lines.append("- [ ] Policy gate enforced: all knowledge DB writes pass deterministic `policy_gate`")
    lines.append("- [ ] **REQUIRED**: Call `/api/finalize-loop` to create Littleboot.md")
    lines.append("- [ ] **VALIDATION**: Confirm Littleboot.md exists at hardcoded path before finalization")
    lines.append("")
    lines.append("---")
    lines.append("")
...
def validate_all_schemas(workspace_root: Path) -> Dict[str, Any]:
    """Validate schemas of key JSON files in the workspace.
    ...
    files_to_check = [
        ...
        workspace_root / "knownissues.json",
    ] + list(workspace_root.glob("milestone_*.json"))
