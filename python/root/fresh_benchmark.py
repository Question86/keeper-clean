...
def make_inputs(n, depth, contiguous=False, seed=0):
    ...
    bus[:, 1] = torch.rand(Nbus, device=DEVICE, dtype=DTYPE, generator=g) * 100.0  # age
    bus[:, 2] = torch.rand(Nbus, device=DEVICE, dtype=DTYPE, generator=g)          # contradiction in [0,1]
    ...
    if contiguous:
        ...
        hist = torch.stack([base + d for d in range(depth)], dim=0).to(torch.int32)
    else:
        hist = torch.randint(0, Nbus, (depth, n), device=DEVICE, generator=g, dtype=torch.int32)
...
def pytorch_baseline(scores_in, bus, hist):
    ...
    # bus[frag, 0] = confidence, bus[frag, 2] = contradiction
    frags = hist  # [D, n]
    conf = bus[frags, 0]   # [D, n]
    contr = bus[frags, 2]  # [D, n]
...
for contiguous in [False, True]:
    for depth in DEPTHS:
        for n in Ns:
            ...
            rows.append((n, depth, contiguous, t_kernel, tk_std, tk_min, tk_max, thr_k, t_torch, tt_std, tt_min, tt_max, thr_t, t_torch / t_kernel))

            print(
                f"n={n:7d} depth={depth:2d} contig={str(contiguous):5s} | "
                f"kernel {t_kernel:.6e}s ±{tk_std:.3e} ({thr_k:8.2f} M/s) | "
                f"torch {t_torch:.6e}s ±{tt_std:.3e} ({thr_t:8.2f} M/s) | "
                f"speedup torch/kernel={t_torch/t_kernel:6.2f}x"
            )

# Save raw metrics to CSV
import csv
with open('benchmark_raw_metrics.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['n', 'depth', 'contiguous', 'kernel_mean_time_s', 'kernel_std_time_s', 'kernel_min_time_s', 'kernel_max_time_s', 'kernel_throughput_Ms', 'torch_mean_time_s', 'torch_std_time_s', 'torch_min_time_s', 'torch_max_time_s', 'torch_throughput_Ms', 'speedup_torch_over_kernel'])
    for row in rows:
        n, depth, contiguous, tk_mean, tk_std, tk_min, tk_max, thr_k, tt_mean, tt_std, tt_min, tt_max, thr_t, speedup = row
        writer.writerow([n, depth, contiguous, tk_mean, tk_std, tk_min, tk_max, thr_k, tt_mean, tt_std, tt_min, tt_max, thr_t, speedup])

print("Raw metrics saved to benchmark_raw_metrics.csv")
print("DONE")