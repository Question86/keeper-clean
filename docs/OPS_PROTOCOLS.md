...
- To ensure deterministic transition, ALWAYS call `/api/confirm-bootstrap` explicitly after deleting _BOOTSTRAP.md.
- This sets the loop to ACTIVE state immediately, rather than relying on implicit detection in `/api/status`.
